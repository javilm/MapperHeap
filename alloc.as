...
; size word : bits 0-13 = whole-block size (incl. both size words)
;             bit 14    = always zero: no block can exceed 3FF8h, so the
;                         size always fits in 14 bits. Spare, if a second
;                         per-block flag is ever wanted.
;             bit 15    = free flag
...
FREEBIT		equ	08000h		; bit 15 of the size word = "free"
					; (bit 14 is always 0 - see the note above)
...
newseg:		call	allocseg	; A=segment, B=slot; CY set = OOM
		...
		derefp	tmpfp		; map the new segment into page 2
					; (write via absolute page-2 addresses
					; below)
		...
		fpcopy	WIN+FENCE+BLKNEXT,freehd	; big.next = curr head
		ld	hl,NULLOFF			; big.prev = null
		ld	(WIN+FENCE+BLKPREV+2),hl	; only the offset field
							; marks null

		fpnull	freehd		; did a head already exist?
		jr	z,newseg.head	; old head was null -> nothing to fix

		derefp	freehd		; else point old head's prev at big blk
					; map old head (remaps page 2, which is
...
newseg.head:	fpcopy	freehd,tmpfp	; head = big block
		or	a		; CY clear = success
		ret
...
flremove:	call	deref		; HL -> block header (input consumed)
		...
		ld	de,BLKNEXT
		add	hl,de
		fpsave	frnext		; frnext = block.next
		pop	hl
		ld	de,BLKPREV
		add	hl,de
		fpsave	frprev		; frprev = block.prev

		; --- prev.next = frnext (or freehd = frnext if prev is null)
		fpnull	frprev
		jr	z,flr.prevnull
		derefp	frprev		; HL -> prev header (remaps page 2)
		ld	de,BLKNEXT
		add	hl,de
		ex	de,hl		; DE -> prev.next (dest in DE - manual)
		ld	hl,frnext
		ld	bc,4
		ldir
		jr	flr.donext
flr.prevnull:	fpcopy	freehd,frnext	; block was the head: freehd = frnext

		; --- next.prev = frprev (skip if next is null)
flr.donext:	fpnull	frnext
		ret	z		; next is null -> done
		derefp	frnext		; HL -> next header
		ld	de,BLKPREV
		add	hl,de
...
halloc.search:	fpcopy	hcur,freehd	; start at the head
halloc.scan:	fpnull	hcur		; end of list?
		jr	z,halloc.grow	; nothing fit -> get a new segment
		derefp	hcur		; HL -> this block's header
		ld	e,(hl)
		inc	hl
		...
		derefp	hcur		; else advance: hcur = hcur.next
		ld	de,BLKNEXT
		add	hl,de
		fpsave	hcur
		jr	halloc.scan
...
halloc.found:	fpcopy	hblk,hcur		; hblk = the found block
		derefp	hblk
		ld	e,(hl)
		inc	hl
		ld	d,(hl)
...
halloc.whole:	ld	hl,hblk		; unlink it from the free list
		call	flremove
		derefp	hblk		; clear the free bit in header...
		inc	hl
		res	7,(hl)
		derefp	hblk		; ...and in footer (at header+size-1)
		ld	de,(hsize)
		add	hl,de
		dec	hl
		res	7,(hl)
		fpcopy	haptr,hblk	; haptr = hblk (allocated = whole blk)
		jr	halloc.done

; --- split: shrink free block to rem, carve allocated block off the back
halloc.split:	derefp	hblk		; free block header = rem, free
		ld	de,(hrem)
		ld	a,e
		ld	(hl),a
		inc	hl
		...
		or	080h
		ld	(hl),a
		derefp	hblk		; free block footer @ header+rem-2
		ld	de,(hrem)
		add	hl,de
		dec	hl
		dec	hl
		...
		or	080h
		ld	(hl),a
		derefp	hblk		; allocated block header @ header+rem,
					; size=need, used
		ld	de,(hrem)
		add	hl,de
		push	hl
...
halloc.done:	ld	hl,(haptr+2)	; header -> payload: offset += 2
		...
		ld	bc,4
		ldir
		ld	hl,(hblocks)	; one more block is out on loan. Only
		inc	hl		; this path counts it: halloc.fail below
		ld	(hblocks),hl	; returns having allocated nothing
		or	a		; CY clear = success
		ret
...
fladd:		fpsave	fablk		; fablk = the block's far pointer
					; (input arrives in HL)
		derefp	fablk		; block.next = current head
		ld	de,BLKNEXT
		add	hl,de
		ex	de,hl
		ld	hl,freehd
		ld	bc,4
		ldir
		derefp	fablk		; block.prev = null
		ld	de,BLKPREV+2
		add	hl,de
		ld	(hl),0ffh
		inc	hl
		ld	(hl),0ffh
		fpnull	freehd		; does a head already exist?
		jr	z,fladd.sethead
		derefp	freehd		; old head.prev = fablk
		ld	de,BLKPREV
		add	hl,de
		ex	de,hl
		ld	hl,fablk
		ld	bc,4
		ldir
fladd.sethead:	fpcopy	freehd,fablk	; head = fablk
		ret
...
hfree:		fpsave	hfblk		; hfblk = caller's far ptr (payload:
					; input arrives in HL)
		ld	hl,(hblocks)	; one fewer on loan. AFTER the fpsave
		dec	hl		; above, not before: HL was the
		ld	(hblocks),hl	; caller's argument until then
		...
		derefp	hfblk		; read the block's size
		ld	e,(hl)
		inc	hl
		ld	d,(hl)
		res	7,d
...
; --- coalesce forward: neighbor at header + size
		...
		add	hl,de
		ld	(hfnext+2),hl
		derefp	hfnext		; read neighbor's size word
		ld	e,(hl)
		inc	hl
		ld	d,(hl)
		bit	7,d		; free?
...
; --- coalesce backward: neighbor's footer is at header - 2
		...
		add	hl,de
		ld	(hfsize),hl
		fpcopy	hfblk,hfprev	; the merged block now starts at prev
hfree.bwddone:
...
hblocks:	defs	2	; blocks handed out by halloc and not yet given
				...
				; equivalent, and see a leak. heapinit zeroes it
